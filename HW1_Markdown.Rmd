...
#Q3.1
CLIP_data <- read.table(file.path("./CLIP_data.BED")
                        ,header=TRUE)
...
gr <- GRanges(seqnames =CLIP_data["chromosome"], IRanges(start=c(CLIP_data["start"]),end=c(CLIP_data["end"])),strand="+")
gr
...
gr <- GRanges(seqnames="chromosome", IRanges(start=start_wo_head,end=end_wo_head),strand="+")
seqnames(gr)
